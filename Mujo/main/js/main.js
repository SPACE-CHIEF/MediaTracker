	...
	for (const key in response.data) {
		...
		fetch(`https://api.themoviedb.org/3/search/movie?api_key=c9e5f6b44a8037f35894aef02579205a&language=en-US&query=${key}&page=1`)
			...
			.then(data => {
				...
				fetch(`https://api.themoviedb.org/3/movie/${movieID}/credits?api_key=c9e5f6b44a8037f35894aef02579205a&language=en-US`)
				.then(response => {
					return response.json();
				})
				.then(credits => {
					...
					for(const member of crew){
						...
						if(member.known_for_department == "Directing"){
							addModal += `<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModal" aria-hidden="true">
				...
				<div class="col-md-6 col-lg-4 col-xl-3 pb-4">
					<div class="card h-100">					
						...
						<div class="card-body">	
							...
							<a class="stretched-link" role="button" data-toggle="modal" data-target="#myModal"></a>
						</div>
					</div>
				</div>
				`
